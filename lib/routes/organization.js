...
(function () {
  ...
  X.organizationRoute = X.AdministrativeRoute.create({
    update: function (xtr, id) {
      X.debug("organization: my update handler was called: %@, %@".f(xtr.get("path"), id));
      X.debug(xtr.get("data"));
      xtr.close();
    },
    delete: function (xtr, id) {
      X.debug("organization: my delete handler was called: %@, %@".f(xtr.get("path"), id));
      xtr.close();
    },
    new: function (xtr) {
      X.debug("organization: my new handler was called: %@".f(xtr.get("path")));
      xtr.close();
    },
